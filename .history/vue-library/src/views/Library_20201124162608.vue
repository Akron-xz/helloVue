  ...
  <div class="library-container">
    ...
    <div class="borrow-box" v-show="!tableDisplay">
      ...
      <div class="borrow-button">
        <el-button v-if="num > 10" type="primary" @click="borrow" disabled>
          已借
        </el-button>
        <el-button v-else-if="num <= 10 && num >= 0" type="primary" @click="borrow">
          有货
        </el-button>
        <el-button v-else type="primary" @click="borrow" disabled style="background-color: grey">
          无货
        </el-button>
...
export default {
  ...
  methods: {
    ...
    // 书籍借阅状态判断
    bookStatus() {
      // 用户ID，在session里面，this.userData[0].userId
      // 书的ID，scope.row，this.bookId
      // 书的数量，scope.row，this.surplusNumber

      // 用户有借阅历史
      if (this.userData[0].userId == borrowList.userId) {
        console.log("已经借阅");
        if (condition) {
  ...
  created() {
    ...
    axios.get("http://localhost:8081/user/bookshelvest").then((res) => {
      this.return = res.data;
    });
    // 获取图书借阅表borrow表的信息
    // axios.get("http://localhost:8081/borrow/list").then((res) => {
